 ...
 *
 * <p>等额本息：月供按标准年金公式 融资金额 × 月利率 × (1+月利率)^期数 / ((1+月利率)^期数 - 1)
 * 计算（保留 2 位小数，HALF_UP），零利率时按融资金额平均分摊；每期利息 = 期初剩余本金 × 月利率，
 * 应还本金 = 月供 - 当期利息，最后一期以剩余本金作为应还本金、本金加当期利息作为应还总额，
 * 用于吸收累计舍入差额。
 *
 * <p>公共规则：
...
public class RentScheduleCalculator {
    ...
    public RentSchedule calculate(BigDecimal financingAmount, BigDecimal nominalAnnualRate,
        ...
        return calculate(financingAmount, nominalAnnualRate, termMonths, firstPaymentDate,
    ...
    public RentSchedule calculate(BigDecimal financingAmount, BigDecimal nominalAnnualRate,
        ...
        return switch (repaymentMethod) {
            case EQUAL_PRINCIPAL -> calculateEqualPrincipal(financingAmount, nominalAnnualRate,
                    termMonths, firstPaymentDate);
            case EQUAL_PAYMENT -> calculateEqualPayment(financingAmount, nominalAnnualRate,
                    termMonths, firstPaymentDate);
        };
    }

    private RentSchedule calculateEqualPrincipal(BigDecimal financingAmount,
        ...
        BigDecimal monthlyRate = monthlyRate(nominalAnnualRate);
    ...
    private RentSchedule calculateEqualPayment(BigDecimal financingAmount,
                                               BigDecimal nominalAnnualRate,
                                               int termMonths, LocalDate firstPaymentDate) {
        BigDecimal monthlyRate = monthlyRate(nominalAnnualRate);
        BigDecimal monthlyPayment = monthlyPayment(financingAmount, monthlyRate, termMonths);
        ...
        for (int period = 1; period <= termMonths; period++) {
            ...
            BigDecimal interestDue = openingPrincipal.multiply(monthlyRate)
                    .setScale(MONEY_SCALE, RoundingMode.HALF_UP);
            BigDecimal principalDue = period < termMonths
                    ? monthlyPayment.subtract(interestDue).min(remaining)
                    : remaining;
            BigDecimal totalDue = principalDue.add(interestDue);
            BigDecimal closingPrincipal = openingPrincipal.subtract(principalDue);
            LocalDate dueDate = firstPaymentDate.plusMonths(period - 1L);
    ...
    /**
     * 标准年金公式月供；零利率时按融资金额平均分摊。
     */
    private BigDecimal monthlyPayment(BigDecimal financingAmount, BigDecimal monthlyRate,
                                      int termMonths) {
        if (monthlyRate.signum() == 0) {
            return financingAmount.divide(BigDecimal.valueOf(termMonths),
                    MONEY_SCALE, RoundingMode.HALF_UP);
        }
        BigDecimal factor = BigDecimal.ONE.add(monthlyRate).pow(termMonths, CALC_CONTEXT);
        return financingAmount.multiply(monthlyRate).multiply(factor)
                .divide(factor.subtract(BigDecimal.ONE), CALC_CONTEXT)
                .setScale(MONEY_SCALE, RoundingMode.HALF_UP);
    }

    private BigDecimal monthlyRate(BigDecimal nominalAnnualRate) {
        return nominalAnnualRate.divide(MONTHS_PER_YEAR, RATE_SCALE, RoundingMode.HALF_UP);
    }
