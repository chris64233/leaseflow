...
LeaseFlow 是一个面向融资租赁场景的 Spring Boot 后端项目，用于承载租赁资产、合同租金及其相关业务数据。当前已实现首个业务功能：创建融资租赁项目，并按等额本金或等额本息方式生成月度租金计划。
...
`repaymentMethod` 可选值为 `EQUAL_PRINCIPAL`（等额本金）和 `EQUAL_PAYMENT`（等额本息）；省略时默认 `EQUAL_PRINCIPAL`，非法取值返回 `400` 并在 `errors` 中指出字段及原因。
...
返回租赁项目详情（含 `repaymentMethod`）及按期次升序排列的租金计划，与创建结果一致；合同不存在返回 `404`。

### 校验与错误响应

- 租赁物编码、合同编号分别唯一，重复返回 `409`。
- 原值、融资金额必须大于 0，且融资金额不得超过原值；名义年利率取值 0 到 1（允许为 0）；期数 1 到 120；首期应还日不得早于起租日。参数或业务校验失败返回 `400`。
- `repaymentMethod` 取值非法（非 `EQUAL_PRINCIPAL`/`EQUAL_PAYMENT`）返回 `400`，错误响应 `errors` 中包含 `repaymentMethod` 字段及原因。
...
- 月供按标准年金公式计算：A = P·r·(1+r)ⁿ ÷ ((1+r)ⁿ − 1)，其中 P 为融资金额、r 为月利率、n 为期数，月供保留 2 位小数（HALF_UP）；零利率时按融资金额平均分摊。
- 每期利息 = 期初剩余本金 × 月利率（保留 2 位小数，HALF_UP），应还本金 = 月供 − 当期利息。
- 最后一期以剩余本金为应还本金、本金加当期利息为应还总额，吸收累计舍入差额，保证本金合计严格等于融资金额、期末本金为 0，且所有金额不为负数。
...
- 租赁物、合同（含实际还款方式）、租金计划通过 JPA 在同一事务中持久化，数据库层设有唯一约束，失败时不留部分数据。
